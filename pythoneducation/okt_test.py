...
def greet_visitor(name): # Функция приветствия посетителя
    return f"Welcome to the zoo, {name}!" # Возвращает строку с приветствием

# Класс, представляющий животное
class Animal: # Класс животного
    def __init__(self, name, species, sound): # Метод инициализации
    ...
    def make_sound(self): # Метод издания звука
        return f"{self.name} the {self.species} says {self.sound}!"

# Экземпляр класса Animal для льва
lion = Animal("Leo", "Lion", "roar") # Создание экземпляра класса Animal для льва
